...
contract MultiProtocolIntegrationTest is Test {
    ...
    function testMultiUserDeposits() public {
        ...
        uint256 shares1 = yieldOptimizer.deposit(
            address(token),
            depositAmount1,
            0
        );
        ...
        uint256 shares2 = yieldOptimizer.deposit(
            address(token),
            depositAmount2,
            0
        );
    ...
    function testWithdrawalFromMultipleProtocols() public {
        ...
        uint256 amountWithdrawn = yieldOptimizer.withdraw(
            address(token),
            withdrawShares,
            0
        );
    ...
    function testRebalancingBetweenProtocols() public {
        ...
        uint256 initialCompound = compoundAdapter.getSharesBalance(
            address(token)
        );
        ...
        // Execute rebalancing
        vm.startPrank(admin);
        IYieldOptimizer.RebalanceParams memory params = IYieldOptimizer
            .RebalanceParams({
                token: address(token),
                targets: new IYieldOptimizer.AllocationTarget[](3),
                maxSlippage: 500, // 5%
    ...
    function testEmergencyStopFunctionality() public {
        uint256 depositAmount = 1000e18;
        ...
        // Verify new deposits are blocked
        vm.startPrank(user2);
        token.approve(address(yieldOptimizer), 500e18);
        vm.expectRevert("Emergency stopped");
        yieldOptimizer.deposit(address(token), 500e18, 0);
        vm.stopPrank();
    }

    function testProtocolManagement() public {
        // Test that protocols are properly managed
        // Just verify that the setup worked correctly
        ...
        // Verify deposit worked (indicates protocols are active)
        assertGt(shares, 0);
        console.log("Protocol management test passed with shares:", shares);
    }
}
