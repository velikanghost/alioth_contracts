...
contract CompoundAdapterTest is Test {
    ...
    function setUp() public {
        ...
        adapter.addSupportedToken(address(0), address(mockCEther)); // ETH
        vm.stopPrank();
    }
    ...
    function testDepositERC20() public {
        ...
        uint256 shares = adapter.deposit(tokenAddress, depositAmount, minShares);
        vm.stopPrank();
    ...
    function testDepositETH() public {
        ...
        vm.startPrank(user);
        uint256 shares = adapter.deposit{value: depositAmount}(address(0), depositAmount, minShares);
        vm.stopPrank();
    ...
    function testWithdrawERC20() public {
        ...
        vm.expectEmit(true, false, false, true);
        emit Withdrawn(tokenAddress, 10e18, withdrawShares); // Expected amount based on exchange rate
        
        uint256 amount = adapter.withdraw(tokenAddress, withdrawShares, minAmount);
        vm.stopPrank();
        
        assertEq(amount, 10e18); // 250e18 * 2e17 / 1e18 = 50e18 but we're mocking different logic
        assertEq(adapter.getSharesBalance(tokenAddress), shares - withdrawShares);
    }
    ...
    function testWithdrawETH() public {
        ...
        uint256 depositAmount = 1 ether;
        vm.startPrank(user);
        uint256 shares = adapter.deposit{value: depositAmount}(address(0), depositAmount, 0);
