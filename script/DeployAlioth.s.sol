...
contract DeployAlioth is Script {
    ...
    address constant CCIP_ROUTER_POLYGON = 0x849c5ED5a80F5B408Dd4969b78c2C8fdf0565Bfe;
    address constant CCIP_ROUTER_ARBITRUM = 0x141fa059441E0ca23ce184B6A78bafD2A517DdE8;
    ...
    address constant USDC_ETHEREUM = 0xA0b86a33E6417c92227dD4b4fE3FDEd7F2426e79;
    ...
    function deployContracts(DeploymentConfig memory config) 
        ...
        console.log("CCIPMessenger deployed at:", address(contracts.ccipMessenger));
        ...
        console.log("YieldOptimizer deployed at:", address(contracts.yieldOptimizer));
        ...
        console.log("CrossChainLending deployed at:", address(contracts.lending));
        
        // Deploy Aave Adapter (if on Ethereum)
        if (block.chainid == 1 || block.chainid == 11155111) {
            address aavePool = block.chainid == 1 ? AAVE_POOL_ETHEREUM : address(0x1); // Mock for testnet
            contracts.aaveAdapter = new AaveAdapter(aavePool, config.admin);
            console.log("AaveAdapter deployed at:", address(contracts.aaveAdapter));
        }
    }

    function setupContracts(DeployedContracts memory contracts, DeploymentConfig memory config) internal {
        ...
        if (address(contracts.aaveAdapter) != address(0)) {
            setupYieldOptimizer(contracts.yieldOptimizer, contracts.aaveAdapter);
        }
    ...
    function setupCCIPMessenger(CCIPMessenger ccipMessenger) internal {
        ...
        ccipMessenger.addSupportedChain(ETHEREUM_SELECTOR, CCIP_ROUTER_ETHEREUM);
        ccipMessenger.addSupportedChain(POLYGON_SELECTOR, CCIP_ROUTER_POLYGON);
        ccipMessenger.addSupportedChain(ARBITRUM_SELECTOR, CCIP_ROUTER_ARBITRUM);
        
        console.log("CCIP Messenger configured with supported chains");
    }

    function setupYieldOptimizer(YieldOptimizer yieldOptimizer, AaveAdapter aaveAdapter) internal {
    ...
    function grantRoles(DeployedContracts memory contracts, address admin) internal {
    ...
    function logDeployment(DeployedContracts memory contracts) internal view {
        ...
        console.log("forge verify-contract", address(contracts.ccipMessenger), "src/core/CCIPMessenger.sol:CCIPMessenger");
        console.log("forge verify-contract", address(contracts.yieldOptimizer), "src/core/YieldOptimizer.sol:YieldOptimizer");
        console.log("forge verify-contract", address(contracts.lending), "src/core/CrossChainLending.sol:CrossChainLending");
        
        if (address(contracts.aaveAdapter) != address(0)) {
            console.log("forge verify-contract", address(contracts.aaveAdapter), "src/adapters/AaveAdapter.sol:AaveAdapter");
        }
...
contract DeployTestnet is Script {
    function run() external {
        ...
        CCIPMessenger ccipMessenger = new CCIPMessenger(mockRouter, admin, feeCollector);
        YieldOptimizer yieldOptimizer = new YieldOptimizer(address(ccipMessenger), admin);
