...
contract CrossChainLending is ICrossChainLending, ReentrancyGuard {
    ...
    modifier onlyUnderwriter() {
        require(
            msg.sender == admin || hasRole(UNDERWRITER_ROLE, msg.sender),
            "Not underwriter"
        );
        _;
    }
    ...
    modifier onlyLiquidator() {
        require(
            msg.sender == admin || hasRole(LIQUIDATOR_ROLE, msg.sender),
            "Not liquidator"
        );
        _;
    }
    ...
    function requestLoan(
        ...
        require(
            supportedCollateral[request.collateralToken],
            "Collateral not supported"
        );
        require(
            supportedBorrowTokens[request.borrowToken],
            "Borrow token not supported"
        );
        ...
        require(
            request.maxInterestRate <= MAX_INTEREST_RATE,
            "Max rate too high"
        );
        ...
        uint256 ltvRatio = MathLib.calculateLTV(
        ...
        returns (uint256 principalPaid, uint256 interestPaid)
    {
        ...
        ERC20(loan.borrowToken).safeTransferFrom(
            msg.sender,
            address(this),
            amount
        );
        ...
        uint256 platformFee = ValidationLib.calculatePercentage(
            interestPaid,
            platformFeeRate
        );
        if (platformFee > 0) {
        ...
        uint256 remainingBalance = loan.borrowAmount +
            _calculateAccruedInterest(loanId);
    ...
    function liquidateLoan(
        ...
        onlyLiquidator
        nonReentrant
        returns (uint256 collateralSeized, uint256 debtCovered)
    {
        ...
        (bool eligible, uint256 currentHealthFactor) = isLiquidationEligible(
            loanId
        );
        ...
        if (requiredCollateralValue >= collateralValue) {
            ...
            collateralSeized = MathLib.min(
                maxCollateralSeized,
                (requiredCollateralValue * loan.collateralAmount) /
                    collateralValue
            );

            uint256 collateralValueSeized = _getTokenValue(
                loan.collateralToken,
                collateralSeized
            );
            debtCovered =
                ((collateralValueSeized - bonus) * totalDebt) /
                debtValue;
        }
     ...
     */
    function routeCollateralToYield(
        uint256 loanId,
        uint256 amount,
        address _yieldOptimizer
    ) external onlyAdmin {
        ...
        IYieldOptimizer(_yieldOptimizer).deposit(
            loan.collateralToken,
            amount,
            0
        );

        emit CollateralRouted(
            loanId,
            loan.collateralToken,
            amount,
            _yieldOptimizer
     ...
     * @param _yieldOptimizer Address of the yield optimizer contract
     */
    function withdrawCollateralFromYield(
        uint256 loanId,
        uint256 amount,
        address _yieldOptimizer
    ) external onlyAdmin {
    ...
    function getLoan(
        uint256 loanId
    ) external view returns (ActiveLoan memory loan) {
        return loans[loanId];
    }
     ...
     */
    function isLiquidationEligible(
        uint256 loanId
     ...
     */
    function getLoansForMonitoring(
    ...
    function _calculateAccruedInterest(
        ...
        return
            MathLib.calculateCompoundInterest(
                loan.borrowAmount,
                loan.interestRate,
                timeElapsed
            );
    ...
    function _routeCollateralToYield(uint256 loanId, uint256 amount) internal {
        ...
        ERC20(loan.collateralToken).safeApprove(
    ...
    function _withdrawCollateralFromYield(
        uint256 loanId,
        uint256 amount
    ) internal {
        ActiveLoan memory loan = loans[loanId];
