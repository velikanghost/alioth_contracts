...
contract CompoundAdapter is IProtocolAdapter, ReentrancyGuard {
    ...
    function getAPY(address token) external view returns (uint256 apy) {
        ...
        try ICToken(cTokenAddr).supplyRatePerBlock() returns (uint256 supplyRatePerBlock) {
            ...
            try comptroller.compSpeeds(cTokenAddr) returns (uint256 compSpeed) {
                if (compSpeed > 0) {
                    ...
                    uint256 compRewardsAPY = compSpeed * BLOCKS_PER_YEAR * 100 / BASE_MANTISSA; // Simplified
                    apy += compRewardsAPY;
                }
            } catch {
    ...
    function getTVL(address token) external view returns (uint256 tvl) {
        ...
        if (cTokenAddr != address(0)) {
            try ICToken(cTokenAddr).balanceOf(address(this)) returns (uint256 cTokenBalance) {
                if (cTokenBalance > 0) {
                    try ICToken(cTokenAddr).exchangeRateStored() returns (uint256 exchangeRate) {
    ...
    function deposit(address token, uint256 amount, uint256 minShares) 
        ...
        if (token == address(0)) {
            ...
            try ICEther(cTokenAddr).mint{value: amount}() {
                uint256 finalBalance = ICToken(cTokenAddr).balanceOf(address(this));
                ...
                revert(string(abi.encodePacked("Compound ETH deposit failed: ", reason)));
            }
        } else {
            // ERC20 token deposit
            ...
            try ICToken(cTokenAddr).mint(amount) returns (uint256 mintResult) {
                ...
                revert(string(abi.encodePacked("Compound deposit failed: ", reason)));
            }
        }
    }
    ...
    function withdraw(address token, uint256 shares, uint256 minAmount) 
        ...
        try ICToken(cTokenAddr).redeem(shares) returns (uint256 redeemResult) {
            ...
            revert(string(abi.encodePacked("Compound withdrawal failed: ", reason)));
        }
    }
    ...
    function harvestYield(address token) external returns (uint256 yieldAmount) {
        ...
        try comptroller.claimComp(address(this)) {
            uint256 finalCompBalance = ERC20(compToken).balanceOf(address(this));
     ...
     */
    function addSupportedToken(address token, address cToken) external onlyAdmin {
