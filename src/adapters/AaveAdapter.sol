...
contract AaveAdapter is IProtocolAdapter, ReentrancyGuard {
    ...
    function getAPY(address token) external view returns (uint256 apy) {
        ...
        try aavePool.getReserveData(token) returns (ReserveData memory reserveData) {
    ...
    function getTVL(address token) external view returns (uint256 tvl) {
        ...
        if (aToken != address(0)) {
            try IAToken(aToken).balanceOf(address(this)) returns (uint256 balance) {
                tvl = balance;
            } catch {
                tvl = 0;
            }
    ...
    function withdraw(address token, uint256 shares, uint256 minAmount) 
        ...
        try aavePool.withdraw(token, shares, address(this)) returns (uint256 withdrawn) {
            amount = withdrawn;
     ...
     */
    function addSupportedToken(address token, address aToken) external onlyAdmin {
        token.validateAddress();
